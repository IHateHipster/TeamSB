...
public class client extends RSApplet {
	...
	public void drawHPOrb() {
 	...
 	public void drawPrayerOrb() {
		...
		/* Draws empty orb */
		prayerBg.drawSprite(186, 49);
		//emptyOrb.drawSprite(171, 49);
		prayerFill.drawSprite(189, 52);
		prayerIcon.drawSprite(193, 56);
		/* Draws current PRAYER text */
		if(prayer <= 100 && prayer >= 75) {
			smallText.method382(65280, 229, cP, 75, true);
		}
		else if(prayer <= 74 && prayer >= 50) {
			smallText.method382(0xffff00, 229, cP, 75, true);
		}
		else if(prayer <= 49 && prayer >= 25) {
			smallText.method382(0xfca607, 229, cP, 75, true);
		}
		else if(prayer <= 24 && prayer >= 0) {
			smallText.method382(0xf50d0d, 229, cP, 75, true);
		}
		/* Draws inside orb sprites */
	}
	...
	private void updateEntities() {
		try{
			...
			if(j < playerCount) {
				...
				if (myPrivilege == 3)
				smallText.method382(0x3399ff, spriteDrawX, "hello", spriteDrawY - 8, true);
				//smallText.method382(65280, x - 13, cHP, y + 23, true);
...
//			ModelDecompressor.loadModels();
			...
			ModelDecompressor.loaditems();
			ModelDecompressor.loadnpcs();
			preloadModels();
			if(!lowMem)
			{
				nextSong = 0;
			...
			prayerBg = new Sprite("Gameframe/Orbs/hitpointsbg");
			...
			hitPointsBg = new Sprite("Gameframe/Orbs/hitpointsbg");
			prayerFill = new Sprite("Gameframe/Orbs/prayerfill");
			prayerIcon = new Sprite("Gameframe/Orbs/prayerIcon");
			heartIcon = new Sprite("Gameframe/Orbs/heart");
			chatArea = new Sprite("Gameframe/Gameframes/chatarea");
			tabArea = new Sprite("Gameframe/Gameframes/tabarea");
			mapArea = new Sprite("Gameframe/Gameframes/maparea");
